            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box">
                        <h4 class="page-title">{{ __('Ajouter une demandes') }}</h4>
                    </div>
                </div>
            </div>
            <div class="row">
                            ...
                            <form action="{{ route('admin.demandes.store') }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="row">
                                    <div class="col-md-5 mb-3">
                                        ...
                                        <select id="stagiaire_id" name="stagiaire_id" class="form-select @error('stagiaire_id') is-invalid @enderror" placeholder="Nom et Prénom">
                                          <option value="" selected>{{ __('Nom et Prénom') }}</option>
                                          @foreach ($stagiaires as $stagiaire )
                                              <option value="{{ $stagiaire->id }}">{{ $stagiaire->nomprenomfr }}</option>
                                          @endforeach
                                        </select>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-5 mb-3">
                                        ...
                                        <input type="file" name="fichers[]"  id="ficher" accept=".xlsx,.xls,image/*,.doc, .docx,.ppt, .pptx,.txt,.pdf" class="form-control @error('fichers') is-invalid @enderror" >
                                        @error('fichers')
                                            <span class="text-danger">
                                                {{ $message }}
                                            </span>
        ...
        $(document).on('click', '#ajouter-row-ficher', function() {
            let ajouterRowFicher = `
                <div class="row container-row">
                    ...
                    <div class="col-md-2 mb-3" style="margin-top: 5px;">
                        <button type="button" class="btn btn-outline-danger mt-3 supprimer-row-ficher" id="supprimer-row-ficher"><span class="mdi mdi-trash-can-outline"></span></button>
                    </div>
                </div>
            `;
